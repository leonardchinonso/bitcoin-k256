...
fn p2pk_pubkey_bytes_different_op_code_returns_none() {
    let key_str = "0411db93e1dcdb8a016b49840f8c53bc1eb68a382e97b1482ecad7b148a6909a5cb2e0eaddfb84ccf9744464f82e160bfa9b8b64f9d4c03f999b8643f656b412a3";
    let key = PublicKey::from_str(key_str).unwrap();
    let different_op_code = Script::builder()
        .push_key(&key)
        .push_opcode(OP_NOP)
        .into_script();
...
fn script_asm() {
    assert_eq!(
        ScriptBuf::from_hex("6363636363686868686800")
            .unwrap()
            .to_asm_string(),
    ...
    assert_eq!(
        ScriptBuf::from_hex("4c").unwrap().to_asm_string(),
        "<unexpected end>"
    );
    assert_eq!(
    ...
    assert_eq!(
        ScriptBuf::from_hex("4d").unwrap().to_asm_string(),
        "<unexpected end>"
    );
    assert_eq!(
...
fn script_buf_collect() {
    assert_eq!(
        &core::iter::empty::<Instruction<'_>>().collect::<ScriptBuf>(),
        Script::new()
    );
...
fn script_p2sh_p2p2k_template() {
    ...
    assert!(
        ScriptBuf::from_hex("a914acc91e6fef5c7f24e5c8b3f11a664aa8f1352ffd87")
            .unwrap()
            .is_p2sh()
    );
    assert!(
    ...
    assert!(
        !ScriptBuf::from_hex("a314acc91e6fef5c7f24e5c8b3f11a664aa8f1352ffd87")
            .unwrap()
            .is_p2sh()
    );
}
...
fn test_iterator() {
    ...
    assert_eq!(
        v_zero,
        vec![(0, Instruction::PushBytes(PushBytes::empty()))]
    );
    ...
    assert_eq!(
        ...
        vec![
            ...
            (2, Instruction::Op(opcodes::OP_NOP3))
        ]
    );
...
fn test_bitcoinconsensus() {
    ...
    spent
        .verify(0, crate::Amount::from_sat(18393430), &spending)
        .unwrap();
}
...
fn defult_dust_value_tests() {
    ...
    assert_eq!(
        script_p2wpkh.minimal_non_dust(),
        crate::Amount::from_sat(294)
    );
    assert_eq!(
    ...
    assert_eq!(
        script_p2pkh.minimal_non_dust(),
        crate::Amount::from_sat(546)
    );
    assert_eq!(
...
fn instruction_script_num_parse() {
    let push_bytes = [
        ...
        (
            PushBytesBuf::from([0xef, 0xbe, 0xad, 0x5e]),
            Some(0x5eadbeef),
        ),
        // Add negative
        (
            PushBytesBuf::from([0xef, 0xbe, 0xad, 0xde]),
            Some(-0x5eadbeef),
        ),
    ];
    let ops = [
