...
impl WitnessProgram {
    ...
    fn new_p2wpkh(program: [u8; 20]) -> Self {
        WitnessProgram {
            ...
            program: ArrayVec::from_slice(&program),
        }
    }
    ...
    pub fn p2tr(internal_key: UntweakedPublicKey, merkle_root: Option<TapNodeHash>) -> Self {
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match *self {
            InvalidLength(len) => write!(
                f,
                "witness program must be between 2 and 40 bytes: length={}",
                len
            ),
            InvalidSegwitV0Length(len) => write!(
                f,
                "a v0 witness program must be either 20 or 32 bytes: length={}",
                len
            ),
        }
    }
